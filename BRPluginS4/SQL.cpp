...
{								   	
	...
	Temp QueryMuOnline[] = {	
		...
		{"ALTER TABLE Character ADD [PatenteWin] [int] DEFAULT (0) NOT NULL"
		,"PatenteWin","Character"},
		...
		{"ALTER TABLE Character ADD [BanChat] [tinyint] DEFAULT (0) NOT NULL"
		,"BanChat","Character"}, 													   
	...
	Temp QueryMeMuOnline[] = {												   																   
		...
		{"ALTER TABLE MEMB_INFO ADD [BanAccTime] [smalldatetime] NULL"
		,"BanAccTime","MEMB_INFO"},	
	};
	if(MeMu)
...
void LoadQuery()
{
	...
	GetPrivateProfileStringA("SQL","SQLLogin","sa",szUser,sizeof(szUser),BR_SQL);
	GetPrivateProfileStringA("SQL","SQLPass","hugo1998",szPassword,sizeof(szPassword),BR_SQL);
	GetPrivateProfileStringA("SQL","SQLDB","MuOnline",szDatabase,sizeof(szDatabase),BR_SQL);
	GetPrivateProfileStringA("SQL","SQLDB2","Me_MuOnline",szDatabase2,sizeof(szDatabase2),BR_SQL);

	if ( MuOnlineQuery.Connect(szDatabase, szUser, szPassword) == TRUE )
	{
		Log.ConsoleOutPut(0,c_Red,t_NULL,"[û] [MuOnline]\tConnection Successfull!");
	}
	else
	{
		Log.ConsoleOutPut(0,c_Red,t_NULL, "[X] [MuOnline]\tError On Connection!");
	}

	if ( Me_MuOnlineQuery.Connect(szDatabase2, szUser, szPassword) == TRUE )
	{
		Log.ConsoleOutPut(0,c_Red,t_NULL,"[û] [MuOnline2]\tConnection Successfull!");
	}
	else
	{
		Log.ConsoleOutPut(0,c_Red,t_NULL, "[X] [Me_MuOnline]\tError On Connection!");
	} 
...
BOOL CQuery::Connect(TCHAR * lpszDNS, TCHAR * lpszUser, TCHAR * lpszPassword)
{
	...
	if ( Result != SQL_SUCCESS && Result != SQL_SUCCESS_WITH_INFO )
	{
		...
		{
			Log.ConsoleOutPut(0,c_Red,t_NULL,  "SQLSTATE:%s, Diagnosis:%s", SqlState, SQLMsgError);
		}
...
BOOL CQuery::ExecQuery(TCHAR * lpszStatement, ...)
{
	...
	{
		Log.ConsoleOutPut(0,c_Red,t_NULL,  "[CQuery] ColCount >= MAX_COLUMNS-1");
		return FALSE;
	}
	...
	{
		Log.ConsoleOutPut(0,c_Red,t_NULL,   "[BRSQL] %s", szStatement);
		this->Close();
		return TRUE;
	}
...
void CQuery::GetAsString(LPTSTR ColName, LPTSTR pOutBuffer)
{
	...
	{
		pOutBuffer[0] = 0;
		Log.ConsoleOutPut(0,c_Red,t_NULL,   "[Query][String]\t\t%s = ERROR", ColName);
	}
}
...
DWORD CQuery::GetAsInteger(LPTSTR ColName)
{
	...
	Log.ConsoleOutPut(0,c_Red,t_NULL,   "[Query][Int]\t\t%s = ERROR", ColName);
	return -1;
}
...
float CQuery::GetAsFloat(LPTSTR ColName)
{
	...
	if ( iIndex != -1 )
	{
		//Log.ConsoleOutPut(1, c_Yellow, t_SQL, "[Query][Float]\t%s = %f", ColName, atof(this->m_SQLData[iIndex]));
		return (float)atof(this->m_SQLData[iIndex]);
	}

	Log.ConsoleOutPut(0,c_Red,t_NULL,   "[Query][Float]\t%s = ERROR", ColName);
	return -1;
}
...
void CQuery::SetAsBinary(LPTSTR lpszStatement, LPBYTE lpBinaryBuffer, SQLUINTEGER BinaryBufferSize)
{
	...
	while ( Result == SQL_NEED_DATA )
	{
		...
		Result = SQLParamData(this->m_hStmt, &pToken);/*False	2	795	V519	The 'Result' variable is assigned values twice successively. Perhaps this is a mistake. Check lines: 459, 460.	IA Julia 1.x.x	query.cpp	460	False*/
...
void CQuery::Disconnect()
{
	...
	if ( this->m_hConnection )/*False	2	796	V581	The conditional expressions of the 'if' operators situated alongside each other are identical. Check lines: 473, 476.	IA Julia 1.x.x	query.cpp	476	False*/
		SQLFreeHandle(SQL_HANDLE_DBC, this->m_hConnection);
