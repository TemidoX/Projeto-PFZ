...
{
		Config.Enabled = GetPrivateProfileInt("DuelManager","DuelEnabled",0, BR_Common);

    if(!Config.Enabled)
	{
		Config.Ranking = GetPrivateProfileInt("DuelManager","DuelRanking",0, BR_Common);
		Config.Logging = GetPrivateProfileInt("DuelManager","DuelLogging",0, BR_Common);
		Config.DuelGate = GetPrivateProfileInt("DuelManager","DuelOutGate",0, BR_Common);
		Config.WinRounds = GetPrivateProfileInt("DuelManager","DuelWinRounds",0, BR_Common);
	}
}
...
void cDuelSystem::Run()
{
    for(short i = 0; i < MAX_DUEL_ROOMS; ++i)
    {
        if(this->g_DuelRooms[i].RoomFree == FALSE)
        {
            ...
            for(short u = 0; u < MAX_DUEL_SEPARATORS; u++)
            {
                ...
                if(gObjIsConnected(this->g_DuelRooms[i].szSeparators[u]->m_Index))
                {
            ...
            if(this->g_DuelRooms[i].szlpObjOne != NULL || this->g_DuelRooms[i].szlpObjTy != NULL)
            {
                if
				(
					gObjIsConnected(this->g_DuelRooms[i].szlpObjOne->m_Index) == FALSE ||
					gObjIsConnected(this->g_DuelRooms[i].szlpObjTy->m_Index)  == FALSE
				)
                {
                    this->RoomReset(i);
            ...
            if(this->g_DuelRooms[i].RoomFree == FALSE && this->g_DuelRooms[i].DuelWaiting == FALSE && this->g_DuelRooms[i].szWinner == FALSE)
            {
                for(short s = 0; s < MAX_DUEL_SEPARATORS; s++)
                {
                    ...
                    {
                        if(PlayersStruct[this->g_DuelRooms[i].szSeparators[s]->m_Index].DUEL_SpecVisible == 1)
                        {
                            ...
                            PlayersStruct[this->g_DuelRooms[i].szSeparators[s]->m_Index].DUEL_SpecVisible = 0;
                        }
                    }
                }
            }
    ...
    for(short n = 0; n < MAX_DUEL_ROOMS; n++)
    {
        ...
        {
            if(gObjIsConnected(this->g_DuelRooms[n].szlpObjOne->m_Index) == FALSE || gObjIsConnected(this->g_DuelRooms[n].szlpObjTy->m_Index)  == FALSE )
            {
                this->RoomReset(n);
...
void cDuelSystem::UpdateDuelScore(short iRoom)
{
    ...
    if(Config.Logging)
    {
		...
		(
			...
			c_Blue,
			t_Duel,
			"[Duel System] Room [%d] Results [%s][%s] VS [%s][%s] Rank [%d] : [%d]",
			iRoom + 1,
			gObj->AccountID,
			gObj->Name,
			lpObj->AccountID,
			lpObj->Name,
            g_DuelRooms[iRoom].szPointsOne,
			g_DuelRooms[iRoom].szPointsTy
		);
    }

	if(g_DuelRooms[iRoom].szPointsOne >= Config.WinRounds)
    {
        if(Config.Ranking)
        {
    ...
    if(g_DuelRooms[iRoom].szPointsTy >= Config.WinRounds)
    {
        if(Config.Ranking)
        {
...
void cDuelSystem::UserDuelInfoReset(LPOBJ lpObj)
{
    ...
    PlayersStruct[aIndex].DUEL_Room = -1;
    lpObj->m_iDuelUser					 = -1;
...
void cDuelSystem::RoomReset(short iRoom, bool dontMove, bool dontSendEnd)
{
    ...
    {
        if(gObjIsConnected(this->g_DuelRooms[iRoom].szlpObjOne->m_Index))
        {
    ...
    {
        if(gObjIsConnected(this->g_DuelRooms[iRoom].szlpObjTy->m_Index))
        {
            if(dontSendEnd == false)
            {
...
void cDuelSystem::KillUserProc(LPOBJ lpObj, LPOBJ lpTarget)
{
    ...
    if(Points >= Config.WinRounds)
    {
        ...
        MessageAll(0, 0, NULL, "Duel: %s Venceu, %s Perdeu.", lpObj->Name, lpTarget->Name);
        ...
        for(short i = 0; i < MAX_DUEL_SEPARATORS; ++i)
        {
            if(this->g_DuelRooms[lpMsg->btRoomIndex].szSeparators[i] == NULL)
            {
                if(gObjMoveGate(lpObj->m_Index, g_DuelGates[lpMsg->btRoomIndex].SeparatorGate))
                {
                    ...
                    if(Config.Logging)
                    {
                        Log.ConsoleOutPut(1, c_Blue, t_Duel,"[Duel System][%s][%s] Spectator join to room: [%d]",lpObj->AccountID, lpObj->Name, lpMsg->btRoomIndex + 1);
                    }
                    ...
                    PlayersStruct[lpObj->m_Index].DUEL_SpecVisible = 1;
...
void cDuelSystem::RecvDuelAnswer(LPOBJ lpObj, PMSG_DUEL_ANSWER_START* lpMsg)
{
    ...
    int iDuelRoom = PlayersStruct[aIndex].DUEL_Room;
        ...
        if(gObjs->m_iDuelUserReserved == lpObj->m_Index)
        {
            ...
            if(!strcmp(szDuelName, szDuelName2))
            {
                ...
                else if (lpMsg->bDuelOK == true)
                {
                    ...
                    MessageAll(0, 0, NULL, "Duelo %s VS %s na sala %d!", lpObj->Name, gObjs->Name, iDuelRoom + 1);
...
void cDuelSystem::RecvDuelRequest(LPOBJ lpObj, PMSG_DUEL_REQUEST_START* lpMsg)
{
    ...
    if(GetPrivateProfileInt("GameServerInfo", "NonPK", 0 , BR_ServerInfo) == 1)
    {
        GCServerMsgStringSend("Dueling is not allowed in NON-PK servers.", lpObj->m_Index, 1);
        return;
    }

    if(GetPrivateProfileInt("GameServerInfo","PkLimitFree", 1, BR_ServerInfo) == 0)
    {
    ...
    PlayersStruct[aIndex].DUEL_Room	 = iDuelRoom;
    int uIndex								 = gObjs->m_Index;
    PlayersStruct[uIndex].DUEL_Room	 = iDuelRoom;
...
bool cDuelSystem::IsOnDuel(int lpObj)
{
    ...
    for(short i = 0; i < MAX_DUEL_ROOMS; i++)
    {
        ...
        if ((room.szlpObjOne != NULL && room.szlpObjOne->m_Index == lpObj) || (room.szlpObjTy != NULL && room.szlpObjTy->m_Index == lpObj))
        {
            if(!room.DuelWaiting) bResult = true;
...
void GSgObjUserDie(OBJECTSTRUCT *lpObj, OBJECTSTRUCT *lpTargetObj)
{
    if(lpObj->Type == OBJECT_USER && lpTargetObj->Type == OBJECT_USER )
    {
        ...
        {
            int aIndex = lpObj->m_Index;
            int DuelRoom = PlayersStruct[aIndex].DUEL_Room;
...
void cDuelSystem::DuelSetInfo( LPOBJ lpObj, int DuelWins, int DuelLoses )
{
    ...
    PlayersStruct[lpObj->m_Index].DUEL_Wins = DuelWins;
    PlayersStruct[lpObj->m_Index].DUEL_Lose = DuelLoses;
    PlayersStruct[lpObj->m_Index].DUEL_AllDuels = All;
}
